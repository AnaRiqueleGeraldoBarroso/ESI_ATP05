...
def tansform_raw_dataset(dataset_file_path: str=None) -> None:
    ...
    # (1) Remover linhas trazem informações de arquivos irrelevante para a tarefa de predição
    irrelevantes = ['examples']
    df = df[~df['FILE'].str.contains('|'.join(irrelevantes), case=False, na=False)]

    #(2) Remover lnhas trazem valores zerados para a maioria das métricas
    colunas_metricas = df.columns[1:]  # ignora a coluna 'FILE'

    zeros_por_linha = (df[colunas_metricas] == 0).sum(axis=1) # Conta a quantidade de valores 0 que te em cada linha

    limite = len(colunas_metricas) // 2  #Calcula a metade das métricas

    df = df[zeros_por_linha <= limite]  # Manter apenas as linhas com menos ou igual a metade das métricas zeradas
    
    #(3) Remover linhas sejam valores muito fora do padrão (outliers)
    media = df[colunas_metricas].mean()
    desvio = df[colunas_metricas].std()
    z_scores = (df[colunas_metricas] - media) / desvio
    df = df[(z_scores.abs() < 3).all(axis=1)]

    # Salva o DataFrame transformado
    df.to_csv(transformed_file_path, sep=';', index=False)
